...
def preprocess(doc_path, meta):
    # If it's a ungenerateable page, don't generate it.
    ...
    # If the output file doesn't exist, it needs to be generated.
    if not os.path.isfile(output_path):
        return True
    # Now check to see if it (or its meta file or the template) is updated. If it is, then keep it around for generation.
    if is_updated(os.path.splitext(doc_path)[0] + ".meta", output_path):
        return True
    if is_updated(args.template_path, output_path):
        return True
...
def finalise():
    ...
    for tag_name in _tag_pages:
        generate_tag_page(tag_name, _tag_pages[tag_name])
...
def generate_document(doc_path, meta, content):
    ...
    canonical = get_canonical(doc_path)
    ...
    with codecs.open(os.path.splitext(doc_path)[0] + ".htm", mode="w", encoding="utf-8") as f:
        f.write(html_output)
